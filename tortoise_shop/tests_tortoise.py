...
class TortoiseShopTests(IsolatedAsyncioTestCase):
    ...
    async def test_tortoise(self):
        ...
        # CREATE
        # Время выполнения составляет приблиз. 17 сек
        # for _ in range(1000):
        ...
        # UPDATE
        # Запрос ORM  с перебором каждой записи занимает приблизит. 5 сек.
        buyers = await Buyer.all()
        for buyer in buyers:
            buyer.balance = 2000
            await buyer.save()

        # # Прямой SQL запрос обновляет приблизит. за 0,3 сек.
        # await Buyer.raw("UPDATE tortoise_buyer SET balance = 2000")
        # buyers = await Buyer.all()
        # for buyer in buyers:
        #     print(f'Баланс: {buyer.balance}')

        # # Запрос ORM c методом bulk_create занимает приблизит. 0.3 сек.
        # buyers = await Buyer.all()
        # for buyer in buyers:
        #     buyer.balance = 2000
